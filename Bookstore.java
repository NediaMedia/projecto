...
public class Bookstore {
    ...
    public void restockProduct(int productId, int quantity) {
        ...
        if (productId < 0 || productId >= totalSize) {
            System.out.println("Invalid product ID");
            return;
        }

        if (productId < bookInventory.size()) {
            Book book = bookInventory.get(productId);
            book.setStock(book.getStock() + quantity);
        } else if (productId < bookInventory.size() + cdInventory.size()) {
            CD cd = cdInventory.get(productId - bookInventory.size());
            cd.setStock(cd.getStock() + quantity);
        } else {
            DVD dvd = dvdInventory.get(productId - bookInventory.size() - cdInventory.size());
            dvd.setStock(dvd.getStock() + quantity);
        }
    }
    ...
    public double inventoryValue() {
        ...
        for (CD cd : cdInventory) {
            totalValue += cd.getPrice() * cd.getStock();
        }

        for (DVD dvd : dvdInventory) {
            totalValue += dvd.getPrice() * dvd.getStock();
        }
...
public Product getProductById(int id) {
    ...
    id -= bookInventory.size();

    if (id < cdInventory.size()) {
        return cdInventory.get(id);
    }

    id -= cdInventory.size();

    if (id < dvdInventory.size()) {
        return dvdInventory.get(id);
    }
